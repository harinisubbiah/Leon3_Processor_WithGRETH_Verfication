...
static inline unsigned int load_bypass(unsigned int addr)
{
    unsigned int tmp;
...
{
    WR(GRETH_MDIO, ((val & 0xFFFF) << 16) | (phy  << MDIO_PHYSHIFT)  | ((reg & 0x1F) << MDIO_REGSHIFT) | MDIO_WRITE);
    mdio_wait();
}
...
static void test_greth_registers(void)
{
    ...
    v = RD(GRETH_CTRL);
    check("TX bit set", v & CTRL_TXEN);
    check("RX bit set", v & CTRL_RXEN);
...
static void test_phy_mdio_read(void)
{
    ...
    check("PHY ID1 not 0xFFFF", id1 == 0xBBCD);
    check("PHY ID2 not 0xFFFF", id2 == 0x9C83);
...
static void test_phy_loopback_mode(void)
{
    ...
    check("PHY loopback bit confirmed via readback",ctrl & PHY_CTRL_LOOPBACK);
}
...
static void test_multi_frame_loopback(void)
{
    ...
    for (i = 0; i < TIMEOUT * NUM_FRAMES; i++) {
        ...
        // bypass cache for STATUS register
        sts = load_bypass(GRETH_STATUS);
        ...
        // bypass cache for ALL descriptor ctrl reads
        int all_done = 1;
        for (j = 0; j < NUM_FRAMES; j++) {
            unsigned int ctrl = load_bypass((unsigned int)&txd[j].ctrl);
            if (ctrl & DESC_EN) { all_done = 0; break; }
        }
    ...
    for (i = 0; i < TIMEOUT * NUM_FRAMES; i++) {
        ...
        int all_done = 1;
        for (j = 0; j < NUM_FRAMES; j++) {
            unsigned int ctrl = load_bypass((unsigned int)&rxd[j].ctrl);
            if (ctrl & DESC_EN) { all_done = 0; break; }
        }
    ...
    for (i = 0; i < NUM_FRAMES; i++) {
        ...
        for (j = 14; j < FRAME_SIZE; j++) 
        {
            unsigned int addr = (unsigned int)&rxbuf[i][j];
            unsigned int word;
            unsigned int byte_offset = addr & 3;
            asm volatile("lda [%1]1, %0" : "=r"(word) : "r"(addr & ~3));
            unsigned char rx_byte = (unsigned char)(word >> (8 * (3 - byte_offset)));
            if (rx_byte != txbuf[i][j]) 
            {        
